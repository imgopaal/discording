function Messages({ messages }: { messages: Message[] }) {
	...
	return (
		<div className="overflow-x-hidden overflow-y-scroll h-[calc(100vh-120px)] max-w-[calc(100vw-310px)] p-4">
			{messages.map((msg: Message) => {
				const { message_id, full_name, imageUrl, imageAlt, message, time } = msg
				return (
					<div key={message_id} className="flex flex-row items-start [&:not(:first-of-type)]:mt-4">
						<div>
							<img src={imageUrl} alt={imageAlt} className="h-10 w-10 rounded-full mr-4" />
						</div>
						<div>
